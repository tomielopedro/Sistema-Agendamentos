            ...
            and_(
                ...
                or_(
                    ...
                    and_(
                        ...
                        Agendamento.data_agendamento + timedelta(minutes=Servico.query.get(Agendamento.servico_id).duracao_minutos) > data_agendamento
                    ),
                    # Novo agendamento termina durante um existente
                    and_(
                        Agendamento.data_agendamento < data_fim,
                        Agendamento.data_agendamento + timedelta(minutes=Servico.query.get(Agendamento.servico_id).duracao_minutos) >= data_fim
                    ),
...
def atualizar_agendamento(agendamento_id):
    ...
    try:
        ...
        if data_agendamento != agendamento.data_agendamento and data_agendamento < datetime.now():
            return jsonify({'erro': 'Não é possível agendar para datas passadas'}), 400
...
def verificar_disponibilidade():
    ...
    try:
        ...
        disponivel = conflitos is None and data_agendamento >= datetime.now()
        
        return jsonify({
            'disponivel': disponivel,
            'data': data_str,
            'servico_id': servico_id,
            'motivo': 'Horário ocupado' if conflitos else ('Data no passado' if data_agendamento < datetime.now() else 'Disponível')
        }), 200
    except Exception as e:
        return jsonify({'erro': str(e)}), 500
